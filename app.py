...
CHMAP: Dict[int, str] = {
    **{i: f"ADC{i}" for i in range(0x0, 0xA)},
    0x0A: "CH_0A", 0x0B: "CH_0B", 0x0C: "CH_0C", 0x0D: "CH_0D", 0x0E: "CH_0E",
...
# Keep a fixed ordered list for logging columns
CHANNELS = [
    *[f"ADC{i}" for i in range(10)],
    "CH_0A","CH_0B","CH_0C","CH_0D","CH_0E",
    "SHT75_X109_T","SHT75_X109_H","SHT75_X110_T","SHT75_X110_H",
    "LPS22HB_AB1_T","LPS22HB_AB1_P","LPS22HB_AB2_T","LPS22HB_AB2_P",
    "AB1_V","AB1_IuA","AB2_V","AB2_IuA","AB3_V","AB3_IuA",
]
...
def _decode_packet(payload: bytes) -> Dict[str, float]:
    ...
    off = 0
    _trig = struct.unpack_from("<I", payload, off)[0]; off += 4
    _gpio = struct.unpack_from("<H", payload, off)[0]; off += 2
    nch   = struct.unpack_from("<B", payload, off)[0]; off += 1
    values: Dict[str, float] = {}
    for _ in range(nch):
        ch     = payload[off]; off += 1
        _stat  = payload[off]; off += 1  # status unused
...
def _read_values_from(ser) -> Dict[str, float]:
    ...
    # 2-byte length (heuristic: prefer sane BE value if LE looks huge)
...
class PdcwReader:
    ...
    def __init__(self, url: str, baud: int, loop: asyncio.AbstractEventLoop, queue: "asyncio.Queue[dict]"):
    ...
    def _run(self):
        ...
        with serial_for_url(self.url, baudrate=self.baud, timeout=READ_TIMEOUT_S, write_timeout=READ_TIMEOUT_S) as ser:
            try:
                ser.reset_input_buffer(); ser.reset_output_buffer()
            except Exception:
                pass
...
class ParquetLogger:
    ...
    def _flush(self):
        if not self._buf or not self._writer:
            return
        cols = list(zip(*self._buf))  # transpose rows->columns
        arrays = [pa.array(cols[0], type=pa.int64())]  # ts_ms
        for i, name in enumerate(CHANNELS, start=1):
            # coerce to float32; None/NaN stays null
...
async def index():
    ...
    if not idx.exists():
        return HTMLResponse("<h1>PDCW Monitor</h1><p><code>static/index.html</code> is missing.</p>", status_code=200)
...
reader: Optional[PdcwReader] = None
