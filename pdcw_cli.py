...
import argparse, struct, time, binascii, sys
...
def read_line_retry(ser, tag='line', retries=3, delay=0.15):
    """Read one CRLF-terminated line with small retries to avoid races."""
...
def parse_packet(payload: bytes):
    ...
    for i in range(nch):
        ...
        if name.endswith('_T'):   disp = f'{val:.2f} °C'
        elif name.endswith('_H'): disp = f'{val:.2f} %RH'
        elif name.endswith('_P'): disp = f'{val:.2f} hPa'
        elif name.endswith('_V'): disp = f'{val:.4f} V'
        elif name.endswith('_IuA'): disp = f'{val:.2f} µA'
        else: disp = f'{val:.6g}'
...
def get_values(url, baud, timeout):
    with open_ser(url, baud, timeout) as ser:
        ...
        if start != b'*':
            ...
            print('Unexpected start:', repr(junk)); sys.exit(1)
...
def main():
    ...
    p_set = sub.add_parser('set-avg'); p_set.add_argument('value', type=int)
    sub.add_parser('get-values')
    p_send = sub.add_parser('send'); p_send.add_argument('text')

    args = ap.parse_args()
    if args.cmd == 'get-avg':       get_avg(args.url, args.baud, args.timeout)
    elif args.cmd == 'set-avg':     set_avg(args.url, args.baud, args.timeout, args.value)
    elif args.cmd == 'get-values':  get_values(args.url, args.baud, args.timeout)
    elif args.cmd == 'send':        send(args.url, args.baud, args.timeout, args.text)
